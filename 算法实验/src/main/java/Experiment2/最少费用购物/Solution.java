package Experiment2.最少费用购物;
import java.util.Scanner;

public class Solution {
    static  final int MAXCODE = 999;//商品编码的最大值
    static final int SALECOMB = 99;//优惠商品组合数
    static final int KIND = 5;     //商品种类
    static final int QUANTITY = 5; //购买某种商品数量的最大值

    static int b;//购买商品种类数
    static int s;//当前优惠组合数
    static int[] num = new int[MAXCODE+1];//记录商品编码与商品种类的对应关系
    static int[] product = new int[KIND+1];//记录不同种类商品的购买数量
    static int[][] offer = new int[SALECOMB+1][KIND+1];//offer[i][j]: 商品组合的优惠价(j=0)；某种优惠组合中某种商品需要购买的数量(j>0)
    static Commodity[] purch = new Commodity[KIND+1];//记录不同商品的购买数量和购买价格
    static int[][][][][] cost = new int[QUANTITY+1][QUANTITY+1][QUANTITY+1][QUANTITY+1][QUANTITY+1];//记录本次购买的总花费
